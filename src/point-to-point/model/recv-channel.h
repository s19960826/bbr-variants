...
public:
   ...
   */
  void Attach (Ptr<RecvNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source RecvNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<const Packet> p, Ptr<RecvNetDevice> src, Time txTime);
  ...
  /**
   * \brief Get RecvNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to RecvNetDevice requested
   */
  Ptr<RecvNetDevice> GetRecvDevice (std::size_t i) const;
...
protected:
   ...
   * \returns Ptr to RecvNetDevice source for the 
   * specified link
   */
  Ptr<RecvNetDevice> GetSource (uint32_t i) const;

  /**
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to RecvNetDevice destination for 
   * the specified link
   */
  Ptr<RecvNetDevice> GetDestination (uint32_t i) const;
...
private:
  ...
  /**
   * \brief Wire model for the RecvChannel
   */
  class Link
  {
public:
    ...
    Ptr<RecvNetDevice> m_src;   //!< First NetDevice
    Ptr<RecvNetDevice> m_dst;   //!< Second NetDevice
  };
