...
#ifndef SEND_NET_DEVICE_H
#define SEND_NET_DEVICE_H
 ...
 *
 * This SendNetDevice class specializes the NetDevice abstract
 * base class.  Together with a SendChannel (and a peer 
 * SendNetDevice), the class models, with some level of 
 ...
 * propagation delay is set in the SendChannel).
 */
class SendNetDevice : public NetDevice
{
public:
  ...
  int i;
  std::vector <uint64_t> idValue;
  std::vector <uint64_t> idTime;
  std::vector <uint64_t> reTime;
  ...
  */
  ...
  /**
   * Construct a SendNetDevice
   *
   * This is the constructor for the SendNetDevice.  It takes as a
  ...
  /**
   * Destroy a SendNetDevice
   *
   * This is the destructor for the SendNetDevice.
   */
  virtual ~SendNetDevice ();
   ...
   */
  bool Attach (Ptr<SendChannel> ch);

  /**
   * Attach a queue to the SendNetDevice.
   *
   * The SendNetDevice "owns" a queue that implements a queueing 
  ...
  /**
   * Attach a receive ErrorModel to the SendNetDevice.
   *
   * The SendNetDevice may optionally include an ErrorModel in
  ...
  /**
   * Receive a packet from a connected SendChannel.
   *
   * The SendNetDevice receives packets from its connected channel
...
private:
   ...
   */
  SendNetDevice& operator = (const SendNetDevice &o);
   ...
   * \param o Other NetDevice
   */
  SendNetDevice (const SendNetDevice &o);
...
private:
  ...
  /**
   * The SendChannel to which this SendNetDevice has been
   * attached.
   */
  Ptr<SendChannel> m_channel;

  /**
   * The Queue which this SendNetDevice uses as a packet source.
   * Management of this Queue has been delegated to the SendNetDevice
