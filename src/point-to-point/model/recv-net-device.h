...
#ifndef RECV_NET_DEVICE_H
#define RECV_NET_DEVICE_H
 ...
 *
 * This RecvNetDevice class specializes the NetDevice abstract
 * base class.  Together with a RecvChannel (and a peer 
 * RecvNetDevice), the class models, with some level of 
 ...
 * propagation delay is set in the RecvChannel).
 */
class RecvNetDevice : public NetDevice
{
public:
  /*
  std::vector <int> send_time_A;
  std::vector <int> recv_time_A;
  std::vector <int> delay_time_A;

  std::vector <int> send_time_B;
  std::vector <int> recv_time_B;
  std::vector <int> delay_time_B;

  uint32_t m_flowIdA = FlowIdTag::AllocateFlowId ();
  uint32_t m_flowIdB = FlowIdTag::AllocateFlowId ();
  */
  
  //set device controller
  Ptr<ControlDecider>   dev_controller;
  void SetDevController(Ptr<ControlDecider> controller);
  //double m_time=0;  //for breakpoint 
  ...
  /**
   * Construct a RecvNetDevice
   *
   * This is the constructor for the RecvNetDevice.  It takes as a
  ...
  /**
   * Destroy a RecvNetDevice
   *
   * This is the destructor for the RecvNetDevice.
   */
  virtual ~RecvNetDevice ();
   ...
   */
  bool Attach (Ptr<RecvChannel> ch);

  /**
   * Attach a queue to the RecvNetDevice.
   *
   * The RecvNetDevice "owns" a queue that implements a queueing 
  ...
  /**
   * Attach a receive ErrorModel to the RecvNetDevice.
   *
   * The RecvNetDevice may optionally include an ErrorModel in
  ...
  /**
   * Receive a packet from a connected RecvChannel.
   *
   * The RecvNetDevice receives packets from its connected channel
...
private:
   ...
   */
  RecvNetDevice& operator = (const RecvNetDevice &o);
   ...
   * \param o Other NetDevice
   */
  RecvNetDevice (const RecvNetDevice &o);
...
private:
  ...
  /**
   * The RecvChannel to which this RecvNetDevice has been
   * attached.
   */
  Ptr<RecvChannel> m_channel;

  /**
   * The Queue which this RecvNetDevice uses as a packet source.
   * Management of this Queue has been delegated to the RecvNetDevice
