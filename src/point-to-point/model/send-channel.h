...
public:
   ...
   */
  void Attach (Ptr<SendNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source SendNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<const Packet> p, Ptr<SendNetDevice> src, Time txTime);
  ...
  /**
   * \brief Get SendNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to SendNetDevice requested
   */
  Ptr<SendNetDevice> GetSendDevice (std::size_t i) const;
...
protected:
   ...
   * \returns Ptr to SendNetDevice source for the 
   * specified link
   */
  Ptr<SendNetDevice> GetSource (uint32_t i) const;

  /**
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to SendNetDevice destination for 
   * the specified link
   */
  Ptr<SendNetDevice> GetDestination (uint32_t i) const;
...
private:
  ...
  /**
   * \brief Wire model for the SendChannel
   */
  class Link
  {
public:
    ...
    Ptr<SendNetDevice> m_src;   //!< First NetDevice
    Ptr<SendNetDevice> m_dst;   //!< Second NetDevice
  };
