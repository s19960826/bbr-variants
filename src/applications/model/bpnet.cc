...
NS_LOG_COMPONENT_DEFINE ("Bpnet");

NS_OBJECT_ENSURE_REGISTERED (Bpnet);

TypeId
Bpnet::GetTypeId (void)
{
  	static TypeId tid = TypeId ("ns3::Bpnet")
	    .SetParent<Object> ()
	    .SetGroupName("Applications")
	    .AddConstructor<Bpnet> ()
	    ;
	return tid;
}
...
Bpnet::backPropagationEpoc()
{
    ...
    for (int i = 0; i < outnode; i++)
    {
        ...
        //std::cout << "fabs: " << outputLayer[i]->value << std::endl;
        error += tmpe * tmpe / 2;
...
void 
Bpnet::training(std::vector<sample> sampleGroups, double threshold)
{
    std::vector<sample> sampleGroup;
    error = 100.f;
    if(sampleGroups.size()>10)
    {
        for(int i=sampleGroups.size()-1; i>sampleGroups.size()-11; i--)
        {
            sampleGroup.push_back(sampleGroups[i]);
        }
    }
    else
    {
    ...
    while(error > threshold and upperBound<100)
...
std::vector<double>
Bpnet::predict(std::vector<sample> testGroup)
{
    ...
    //std::cout << "output: " << (testGroup[0].out)[0] << " " << (testGroup[0].out)[1] << " " << (testGroup[0].out)[2] << std::endl;
    return(testGroup[0].out);
}

void 
Bpnet::setInput(std::vector<double> sampleIn)
{
    for (int i = 0; i < innode; i++) inputLayer[i]->value = sampleIn[i];
}

void 
Bpnet::setOutput(std::vector<double> sampleOut)
{
