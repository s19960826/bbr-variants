  ...
  if (sock.m_congestionControl)
    {
      ...
      //m_congestionControl->SetController(m_controller);
    }
...
void
TcpSocketBase::SetController(Ptr<ControlDecider> controller)
{
    m_controller = controller;
    m_congestionControl->SetController(m_controller);
    initController = initController+1;
  ...
  if (InetSocketAddress::IsMatchingType (address))
    {
      ...
      m_tcb->m_address = transport.GetPort ();
...
TcpSocketBase::DupAck ()
{
  ...
  m_controller->ReTime(m_tcb->m_address, Now().GetMicroSeconds());
      ...
      // of RecoveryPoint.
      ...
      else
        {
          ...
          if (exitedFastRecovery)
            ...
            {
              m_congestionControl->IncreaseWindow (m_tcb, segsAcked);
              //std::cout << "increase window" << std::endl;
  ...
  if (!m.IsZero ())
    {
      ...
      //std::cout << "RTT: " << m_rtt->GetEstimate () << std::endl;
      if(initRttCount>1)
      {
        m_controller->RttValue(m_tcb->m_address, (tmp).GetMicroSeconds());
      }
