...
//set controller
void
ProtoCon::SetController (Ptr<ControlDecider> controller, uint16_t tuple)
{
  m_controller = controller;
  m_tuple = tuple;
  sent_manager_.SetController(m_controller, m_tuple);
}
...
void ProtoCon::Retransmit(uint32_t id,StreamOffset off,ByteCount len,bool fin,TransType tt){
    ...
    packet_writer_->SendTo(src,writer.length(),peer_);
    m_controller->ReTime(m_tuple, Now().GetMicroSeconds());
    }
}
