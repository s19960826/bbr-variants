...
//set controller
void
SendPacketManager::SetController (Ptr<ControlDecider> controller, uint16_t tuple)
{
...
bool SendPacketManager::MaybeUpdateRTT(PacketNumber largest_acked,
    ...
    uint32_t rtt=rtt_stats_.smoothed_rtt().ToMicroseconds();
    m_controller->RttValue(m_tuple, rtt);
    return true;
}
