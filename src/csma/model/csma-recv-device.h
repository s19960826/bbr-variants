...
#ifndef CSMA_RECV_DEVICE_H
#define CSMA_RECV_DEVICE_H

#include <cstring>
#include "ns3/node.h"
#include "ns3/recv-backoff.h"
 ...
 */
class CsmaRecvDevice : public NetDevice 
{
public:
  ...
  /**
   * Construct a CsmaRecvDevice
   *
   * This is the default constructor for a CsmaRecvDevice.
   */
  CsmaRecvDevice ();

  /**
   * Destroy a CsmaRecvDevice
   *
   * This is the destructor for a CsmaRecvDevice.
   */
  virtual ~CsmaRecvDevice ();
   ...
   *
   * The function Attach is used to add a CsmaRecvDevice to a CsmaRecvChannel.
   *
   * \see SetDataRate ()
   ...
   */
  bool Attach (Ptr<CsmaRecvChannel> ch);

  /**
   * Attach a queue to the CsmaRecvDevice.
   *
   * The CsmaRecvDevice "owns" a queue.  This queue may be set by higher
  ...
  /**
   * Attach a receive ErrorModel to the CsmaRecvDevice.
   *
   * The CsmaRecvDevice may optionally include an ErrorModel in
  ...
  /**
   * Receive a packet from a connected CsmaRecvChannel.
   *
   * The CsmaRecvDevice receives packets from its connected channel
   ...
   * \param sender the CsmaRecvDevice that transmitted the packet in the first place
   */
  void Receive (Ptr<Packet> p, Ptr<CsmaRecvDevice> sender);
   ...
   *
   */
  void SetEncapsulationMode (CsmaRecvDevice::EncapsulationMode mode);
   ...
   */
  CsmaRecvDevice::EncapsulationMode  GetEncapsulationMode (void);
...
private:
   ...
   * operator for CsmaRecvDevice objects.
   * \param o object to copy
   * \returns the copied object
   */
  CsmaRecvDevice &operator = (const CsmaRecvDevice &o);

  /**
   * Copy constructor is declared but not implemented.  This disables the
   * copy constructor for CsmaRecvDevice objects.
   * \param o object to copy
   */
  CsmaRecvDevice (const CsmaRecvDevice &o);
   ...
   * the CsmaRecvDevice to begin the process of sending a packet
   ...
   *
   * \see CsmaRecvChannel::TransmitStart ()
   ...
   *
   * \see CsmaRecvChannel::TransmitEnd ()
  ...
  /**
   * The CsmaRecvChannel to which this CsmaRecvDevice has been
   * attached.
   * \see class CsmaRecvChannel
   */
  Ptr<CsmaRecvChannel> m_channel;

  /**
   * The Queue which this CsmaRecvDevice uses as a packet source.
   * Management of this Queue has been delegated to the CsmaRecvDevice
  ...
  /**
   * Default Maximum Transmission Unit (MTU) for the CsmaRecvDevice
   */
