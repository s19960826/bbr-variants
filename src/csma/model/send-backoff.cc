...
#include "send-backoff.h"
#include "ns3/log.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("SendBackoff");
...
SendBackoff::SendBackoff(Time slotTime, uint32_t minSlots, uint32_t maxSlots, uint32_t ceiling, uint32_t maxRetries)
{
  m_slotTime = slotTime;
  m_minSlots = minSlots;
  m_maxSlots = maxSlots;
...
Time
SendBackoff::GetBackoffTime (void)
{
  uint32_t ceiling;
...
void 
SendBackoff::ResetBackoffTime (void)
{
  m_numBackoffRetries = 0;
}

bool 
SendBackoff::MaxRetriesReached (void) 
{
  return (m_numBackoffRetries >= m_maxRetries);
}

void 
SendBackoff::IncrNumRetries (void) 
{
  m_numBackoffRetries++;
}

int64_t
SendBackoff::AssignStreams (int64_t stream)
{
