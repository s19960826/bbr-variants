...
#ifndef CSMA_RECV_CHANNEL_H
#define CSMA_RECV_CHANNEL_H
...
class CsmaRecvRec {
public:
  Ptr< CsmaRecvDevice > devicePtr; //!< Pointer to the net device
   ...
   */
  CsmaRecvRec(Ptr< CsmaRecvDevice > device);

  /**
   * Copy constructor
   * \param o the object to copy
   */
  CsmaRecvRec (CsmaRecvRec const &o);
 ...
 */
class CsmaRecvChannel : public Channel 
{
public:
  ...
  /**
   * \brief Create a CsmaRecvChannel
   */
  CsmaRecvChannel ();
  /**
   * \brief Destroy a CsmaRecvChannel
   */
  virtual ~CsmaRecvChannel ();
   ...
   */
  int32_t Attach (Ptr<CsmaRecvDevice> device);
   ...
   * can't be found.
   */
  bool Detach (Ptr<CsmaRecvDevice> device);
   ...
   * channel or can't be found.
   */
  bool Reattach (Ptr<CsmaRecvDevice> device);
   ...
   * number is needed
   */
  int32_t GetDeviceNum (Ptr<CsmaRecvDevice> device);
  ...
  /**
   * \return Get a CsmaRecvDevice pointer to a connected network device.
   *
   ...
   * with deviceId i.
   */
  Ptr<CsmaRecvDevice> GetCsmaDevice (std::size_t i) const;
...
private:
   ...
   * copy constructor for CsmaRecvChannel objects.
   * \param o object to copy
   */
  CsmaRecvChannel (CsmaRecvChannel const &o);

  /**
   * Operator = is declared but not implemented.  This disables the assignment
   * operator for CsmaRecvChannel objects.
   * \param o object to copy
   * \returns the copied object
   */
  CsmaRecvChannel &operator = (CsmaRecvChannel const &o);
   ...
   */
  std::vector<CsmaRecvRec> m_deviceList;
