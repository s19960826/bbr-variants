...
static int ip=1;
static NodeContainer BuildExampleTopo (Ptr<ControlDecider> controller, uint64_t bps,
                                       uint32_t msDelay,
                                       uint32_t msQdelay,bool enable_random_loss)
{
    ...
    pointToPoint.SetHelpController(controller);
    pointToPoint.SetDeviceAttribute ("DataRate", DataRateValue  (DataRate (bps)));
    pointToPoint.SetDeviceAttribute("Mtu", UintegerValue(DEFAULT_PACKET_SIZE));
    pointToPoint.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (msDelay)));
    auto bufSize = std::max<uint32_t> (1, bps * msQdelay / 8000000);
...
int main(int argc, char *argv[]){
	...
	if(cc_tmp==std::string("bbr")){
		...
		cc=kBBRPlus;
		cc1=cc;
	}else if(cc_tmp==std::string("tsunami")){
		cc=kTsunami;;
		cc1=cc;
    ...
    NodeContainer nodes = BuildExampleTopo (m_controller, linkBw, msDelay, msQDelay,enable_random_loss);
	...
	trace1.Log(log,DqcTraceEnable::E_DQC_OWD|DqcTraceEnable::E_DQC_BW|DqcTraceEnable::E_DQC_RTT|DqcTraceEnable::E_DQC_GOODPUT|DqcTraceEnable::E_DQC_STAT);
	test_pair++;
	InstallDqc(m_controller, cc,nodes.Get(0),nodes.Get(1),sendPort,recvPort,appStart,appStop,&trace1,max_bps,cc_id);
	cc_id++;

	DqcTrace trace2;
	log=log_common+std::to_string(test_pair);
	trace2.Log(log,DqcTraceEnable::E_DQC_OWD|DqcTraceEnable::E_DQC_BW|DqcTraceEnable::E_DQC_RTT|DqcTraceEnable::E_DQC_GOODPUT|DqcTraceEnable::E_DQC_STAT);
	test_pair++;
	InstallDqc(m_controller, cc,nodes.Get(0),nodes.Get(1),sendPort+1,recvPort+1,appStart+40,appStop,&trace2,max_bps,cc_id);
	cc_id++;

	DqcTrace trace3;
	log=log_common+std::to_string(test_pair);
	trace3.Log(log,DqcTraceEnable::E_DQC_OWD|DqcTraceEnable::E_DQC_BW|DqcTraceEnable::E_DQC_RTT|DqcTraceEnable::E_DQC_GOODPUT|DqcTraceEnable::E_DQC_STAT);
	test_pair++;
	InstallDqc(m_controller, cc,nodes.Get(0),nodes.Get(1),sendPort+2,recvPort+2,appStart+80,appStop,&trace3,max_bps,cc_id);
	cc_id++;
