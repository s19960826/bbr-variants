...
void ns3_rtt(Ptr<ControlDecider> controller, int ins,std::string algo,DqcTraceState *stat,int sim_time=200,int loss_integer=0){
    ...
    RecvHelper recvp2p;
    sendp2p.SetHelpController(controller);
    p2p.SetHelpController(controller);
    recvp2p.SetHelpController(controller);

    TrafficControlHelper tch;
    //L0
    bufSize =std::max<uint32_t> (1,config[0].bps * config[0].msQdelay/8000000);
    sendp2p.SetQueue ("ns3::DropTailQueue<Packet>",
                           "MaxSize", QueueSizeValue (QueueSize (std::to_string(bufSize)+"p")));
    sendp2p.SetDeviceAttribute("Mtu", UintegerValue(DEFAULT_PACKET_SIZE));
    sendp2p.SetDeviceAttribute ("DataRate", DataRateValue(DataRate (config[0].bps)));
    sendp2p.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (config[0].msDelay)));
    NetDeviceContainer devn0n2 = sendp2p.Install (n0n2);
    ...
    sendp2p.SetQueue ("ns3::DropTailQueue<Packet>",
                           "MaxSize", QueueSizeValue (QueueSize (std::to_string(bufSize)+"p")));
    sendp2p.SetDeviceAttribute("Mtu", UintegerValue(DEFAULT_PACKET_SIZE));
    sendp2p.SetDeviceAttribute ("DataRate", DataRateValue(DataRate (config[3].bps)));
    sendp2p.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (config[3].msDelay)));
    NetDeviceContainer devn1n2 = sendp2p.Install (n1n2);
    ...
    recvp2p.SetQueue ("ns3::DropTailQueue<Packet>",
                           "MaxSize", QueueSizeValue (QueueSize (std::to_string(bufSize)+"p")));
    recvp2p.SetDeviceAttribute("Mtu", UintegerValue(DEFAULT_PACKET_SIZE));
    recvp2p.SetDeviceAttribute ("DataRate", DataRateValue(DataRate (config[2].bps)));
    recvp2p.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (config[2].msDelay)));
    NetDeviceContainer devn3n4 = recvp2p.Install (n3n4);
    ...
    recvp2p.SetQueue ("ns3::DropTailQueue<Packet>",
                           "MaxSize", QueueSizeValue (QueueSize (std::to_string(bufSize)+"p")));
    recvp2p.SetDeviceAttribute("Mtu", UintegerValue(DEFAULT_PACKET_SIZE));
    recvp2p.SetDeviceAttribute ("DataRate", DataRateValue(DataRate (config[4].bps)));
    recvp2p.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (config[4].msDelay)));
    NetDeviceContainer devn3n5 = recvp2p.Install (n3n5);
