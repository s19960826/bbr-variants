...
void ns3_rtt(Ptr<ControlDecider> controller, int ins,std::string algo,DqcTraceState *stat,int sim_time=200,int loss_integer=0){
    ...
    uint32_t msQdelay = 2*TOPO_DEFAULT_PDELAY;
    double randomloss = 0;
    uint16_t sendPort=1000;
    uint16_t recvPort=5000;
    int m = 1; //for schedule the level of bandwidth changes
    ...
    controller -> randomloss = randomloss;
    ...
    for(int i =1; i<20; i++)
    {
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn0n2.Get(0),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn0n2.Get(1),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
	
	Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn1n2.Get(0),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn1n2.Get(1),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
	
	Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn2n3.Get(0),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn2n3.Get(1),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
	
	Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn3n4.Get(0),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn3n4.Get(1),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
	
	Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn3n5.Get(0),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
        Simulator::Schedule(Seconds(i),&NetDevice::SetAttribute,devn3n5.Get(1),"DataRate",DataRateValue(DataRate (m*i*1000000+linkBw)));
...
int main (int argc, char *argv[]){
    ...
    int sim_time=200;
    int ins[]={1,2,3};
    //char *algos[]={"bbr","bbrplus","tsunami","bbrv2"};
    char *algos[]={"bbrrand", "bbradd"};
